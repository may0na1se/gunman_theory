...
export const CARD_INFO: Record<ActiveCardType, { desc: string; needTarget: boolean; cost?: number }> = {
    ...
    '방탄복': { desc: '총에 맞아도 죽지 않음 (1회).', needTarget: false, cost: 7 },
    '도주': { desc: '현재 판돈의 20%를 가지고 라운드 즉시 이탈 및 생존.', needTarget: false },
    ...
    '탄약병': { desc: '사용 즉시 자신의 격발 확률이 90%가 됨.', needTarget: false },
    '저주': {
        desc: "대상 플레이어의 확률을 50% 감소시킵니다. (최소 10%)",
        needTarget: false
    },
    '보험': { desc: '자신이 사망 시 80달러를 지급받음.', needTarget: false, cost: 10 },
    '파괴': { desc: '모든 플레이어의 손에 있는 액티브 카드를 제거함.', needTarget: false },
    '발악': { desc: '격발 확률 25% 감소 대신, 이번 턴 사격이나 카드 획득 시 턴을 1회 소모하지 않습니다.', needTarget: false }
};
